...
final class ProductPriceControllerTest extends TestCase {
	...
	public function testBatchUpsertUpdatesExistingPrice(): void {
		$this->mockUser('alice');
		$this->productMapper->method('findByIdAndOwner')->willReturn($this->product('p1', 'Milk'));
		$this->storeMapper->method('findByIdAndOwner')->willReturn($this->store('s1', 'Store'));
		...
		$existing->setValue(0.99);
		$existing->setPriceDate(new \DateTime('2026-08-01', new \DateTimeZone('UTC')));
		$existing->setCreatedAt(new \DateTime('2026-08-01', new \DateTimeZone('UTC')));

		$this->priceMapper->method('findByProductAndStore')->willReturn($existing);

		$this->db->expects($this->once())->method('beginTransaction');
		$this->db->expects($this->once())->method('commit');

		$this->priceMapper->expects($this->once())->method('update')->with($existing);
		$this->priceMapper->expects($this->never())->method('insert');
		...
		$this->assertSame(Http::STATUS_OK, $response->getStatus());
		$data = $response->getData()['prices'];
		$this->assertCount(1, $data);
		$this->assertSame('price-1', $data[0]['id']);
		$this->assertSame(1.99, $data[0]['value']);
	}

	public function testBatchUpsertRejectsUnknownStore(): void {
		$this->mockUser('alice');
		$this->productMapper->method('findByIdAndOwner')->willReturn($this->product('p1', 'Milk'));
		$this->storeMapper->method('findByIdAndOwner')->willReturn(null);

		$response = $this->controller->batchUpsert([
			['productId' => 'p1', 'storeId' => 'unknown-store', 'value' => 1.5, 'date' => '2026-09-01T10:00:00+00:00'],
		]);

		$this->assertSame(Http::STATUS_UNPROCESSABLE_ENTITY, $response->getStatus());
	}

	public function testBatchUpsertRejectsMissingValue(): void {
		$this->mockUser('alice');
		$this->productMapper->method('findByIdAndOwner')->willReturn($this->product('p1', 'Milk'));

		$response = $this->controller->batchUpsert([
			['productId' => 'p1', 'date' => '2026-09-01T10:00:00+00:00'],
		]);

		$this->assertSame(Http::STATUS_UNPROCESSABLE_ENTITY, $response->getStatus());
	}

	public function testBatchUpsertRejectsNonNumericValue(): void {
		$this->mockUser('alice');
		$this->productMapper->method('findByIdAndOwner')->willReturn($this->product('p1', 'Milk'));

		$response = $this->controller->batchUpsert([
			['productId' => 'p1', 'value' => 'abc', 'date' => '2026-09-01T10:00:00+00:00'],
		]);

		$this->assertSame(Http::STATUS_UNPROCESSABLE_ENTITY, $response->getStatus());
	}

	public function testBatchUpsertRejectsValueExceedingMaxDecimal(): void {
		$this->mockUser('alice');
		$this->productMapper->method('findByIdAndOwner')->willReturn($this->product('p1', 'Milk'));

		$response = $this->controller->batchUpsert([
			['productId' => 'p1', 'value' => 99999999999.99, 'date' => '2026-09-01T10:00:00+00:00'],
		]);
